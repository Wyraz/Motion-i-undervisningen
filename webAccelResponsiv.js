...
let bottomHits = 0;
let minOverskrift;
let øvelser = ["", "Lav engle hop", "Lav squads", "Lav armbøjninger"];
...
function setup() {
    ...
    minOverskrift = select('#overskrift').elt;
    console.log(minOverskrift);
    console.log("test");
}
...
function draw() {
        ...
        if (y >= height || y <= 0) {
            ...
            if (y >= height) {
                bottomHits++;
                if (bottomHits >= 1) {
                    minOverskrift.innerHTML = øvelser[bottomHits];
                    // Ændre farven på canvas, når bolden rammer bunden 5 gang
                    r = random(0, 256);
                    g = random(0, 256);
                    b = random(0, 256);
                    //bottomHits = 0; // Nulstil antallet af bundhits
                }
            }
        }
        if (accelerationX > 70) {
